...
app.post('/api/pix/criar', async (req, res) => {
    try {
          ...
          if (!amount || amount <= 0) return res.status(400).json({ erro: 'Valor invalido' })
          if (!external_id) return res.status(400).json({ erro: 'external_id obrigatorio' })
          if (!NEXUSPAG_API_KEY) return res.status(500).json({ erro: 'API Key nao configurada' })
          const webhookUrl = process.env.RAILWAY_PUBLIC_DOMAIN
            ? `https://${process.env.RAILWAY_PUBLIC_DOMAIN}/api/webhook/nexuspag`
                  : null
          const nexusRes = await fetch(`${NEXUSPAG_BASE_URL}/api/pix/create`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json', 'x-api-key': NEXUSPAG_API_KEY },
                  body: JSON.stringify({
          ...
          if (!nexusRes.ok) return res.status(nexusRes.status).json({ erro: nexusData.message || 'Erro Pix' })
          pedidos[external_id] = { status: 'aguardando', pixId: nexusData.id || nexusData.txid, nome, telefone, endereco, mensagem, items, total: amount, criadoEm: new Date().toISOString() }
          console.log(`Pix criado: ${external_id} | R$ ${amount} | ${nome}`)
          res.json({ sucesso: true, pix: nexusData })
    } catch (err) {
...
app.get('/api/pix/status/:id', async (req, res) => {
    try {
          const { id } = req.params
          if (!NEXUSPAG_API_KEY) return res.status(500).json({ erro: 'API Key nao configurada' })
          const r = await fetch(`${NEXUSPAG_BASE_URL}/api/pix/${id}`, { headers: { 'x-api-key': NEXUSPAG_API_KEY } })
          if (!r.ok) return res.status(r.status).json({ erro: 'Pix nao encontrado' })
          const data = await r.json()
          res.json({ id: data.id || data.txid, status: data.status, pago: isPago(data.status) })
    } catch (err) {
...
function isPago(status) {
    ...
    return ['paid','confirmed','completed','approved','payment.confirmed'].some(v => status.toLowerCase().includes(v))
}
...
app.post('/api/webhook/nexuspag', (req, res) => {
    try {
          if (NEXUSPAG_WEBHOOK_SECRET) {
                  ...
                  if (!sig) return res.status(401).send('Unauthorized')
                  const ts = sig.split(',')[0].slice(2)
          ...
          if (tipo === 'payment.confirmed' || isPago(dados.status)) {
                  const externalIrequire('dotenv').config()
const express = require('express')
const cors = require('cors')
const crypto = require('crypto')
const fetch = require('node-fetch')
...
app.listen(PORT, () => {
  ...
  console.log(`   Webhook:  ${NEXUSPAG_WEBHOOK_SECRET ? '✅ HMAC configurado' : '⚠️  Webhook secret não configurado'}`)
  console.log(``)
})d = dados.external_id || dados.externalId
                  if (externalId && pedidos[externalId]) {
                            pedidos[externalId].status = 'pago'
                            pedidos[externalId].pagoEm = new Date().toISOString()
                  }
...
app.get('/api/pedidos', (req, res) => {
    const lista = Object.entries(pedidos)
      .map(([id, p]) => ({ external_id: id, ...p }))
      .sort((a, b) => new Date(b.criadoEm) - new Date(a.criadoEm))
    res.json({ total: lista.length, pedidos: lista })
})

app.get('/', (req, res) => {
    res.json({ status: 'ok', servico: 'ChocoCesta Backend', versao: '1.0.0' })
})

app.listen(PORT, () => {
    console.log(`ChocoCesta Backend na porta ${PORT}`)
    console.log(`NexusPag: ${NEXUSPAG_API_KEY ? 'OK' : 'NAO configurada'}`)
})
